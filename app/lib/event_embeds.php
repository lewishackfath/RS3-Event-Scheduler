...
function truncateDiscordEmbedText(string $value, int $limit): string
{
    $value = trim($value);
...
function collectWeeklySummaryPosterEvents(array $events): array
{
    $posterEvents = [];
    $seenPosterUrls = [];
    ...
    usort($posterEvents, static function (array $a, array $b): int {
...
function buildWeeklySummaryPosterEmbeds(array $events, int $maxEmbeds = 9): array
{
    if ($maxEmbeds <= 0) {
        return [];
    }
    ...
    foreach (collectWeeklySummaryPosterEvents($events) as $posterEvent) {
        if (count($posterEmbeds) >= $maxEmbeds) {
            break;
        }

        $event = $posterEvent['event'];
        $local = $posterEvent['local'];
        $eventName = trim((string) ($event['event_name'] ?? 'Event'));
        if ($eventName === '') {
            $eventName = 'Event';
        }
        ...
        $posterEmbeds[] = [
            'title' => truncateDiscordEmbedText($local->format('D j M') . ' — ' . $eventName, 256),
            'description' => 'Event poster',
            'color' => hexColourToInt((string) ($brand['embed_colour'] ?? '#5865F2')),
            'image' => [
                'url' => $posterEvent['poster_url'],
            ],
        ];
    }
...
function buildWeeklySummaryEmbeds(array $events, DateTimeImmutable $weekStartLocal, int $maxEmbeds = 10): array
{
    $summaryEmbed = buildWeeklySummaryEmbed($events, $weekStartLocal);
    $maxPosterEmbeds = max(0, $maxEmbeds - 1);
    $posterEvents = collectWeeklySummaryPosterEvents($events);
    $posterEmbeds = buildWeeklySummaryPosterEmbeds($events, $maxPosterEmbeds);
    $posterCount = count($posterEvents);
    $shownPosterCount = count($posterEmbeds);

    if ($posterCount > 0) {
        $posterText = $posterCount === $shownPosterCount
            ? 'Event posters are shown below the weekly schedule.'
            : 'Showing the first ' . $shownPosterCount . ' of ' . $posterCount . ' event posters below. Discord allows up to ' . $maxEmbeds . ' embeds in one message.';
    ...
    return array_merge([$summaryEmbed], $posterEmbeds);
}
